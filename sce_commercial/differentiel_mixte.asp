...
<html lang="en" dir="ltr">
	<head>
		...
		<!-- Title -->
		<title> Paramétrage différentiel mixte - SIGEC4</title>
	...
	<body class="app sidebar-mini rtl">
				...
				<div class="content-area">
					<div class="container">
						...
						<div class="page-header">
							<h4 class="page-title text-info"> Différentiel Mixte - Paramétrage</h4>
							...
							<div class="col-md-12 col-lg-12">
                                <div class="card">
                                    <div class="card-header">
                                        <div class="card-title">Différentiel Mixte - Paramétrage </div>
                                    </div>
                                    <div class="card-body">
											...
											<div class="tab-menu-heading">
												<div class="tabs-menu ">
													...
													<ul class="nav panel-tabs">
														<li class="">
															<a href="#tab1" data-toggle="tab"> Différentiels </a>
														</li>
														<li><a href="#tab2" class="active" data-toggle="tab">Nouveau </a></li>
        ...
        <script  type="text/javaScript">
            ...
            $(function () {
                ...
                _differentiel_mixte_new('#wFrm');
                ComboListe(<%=(right(year(Now),2)+1)%>, 0, 0, 'CboRecolte', '#Recolte', 'Récolte');
                GetDifferentiels('<%=(right(year(Now),2)+1)%>', $('#Produit').val(), $('#xParite').val(), 'ListeDiffMixte', '#wListe');
				//differentiel_liste('<%=(right(year(Now),2)+1)%>', $('#Produit').val(), $('#xParite').val(), '#wListe');

                $('#Recolte,#Produit,#xParite').change(function(){
                	GetDifferentiels($('#Recolte').val(), $('#Produit').val(), $('#xParite').val(), 'ListeDiffMixte', '#wListe'); 
                });

                $('a[href="#tab2"]').click(function(){
                    ComboListe(0, 0, 0, 'CboCamp', '#CampCV', 'Campagne');
                	ComboListe(0, 0, 0, 'CboRecolte', '#RcltStock', 'Récolte');
                });
                ...
                $('#tab2').on('change','#PariteCV,#CampCV,#PariteStock,#RcltStock', function(){
                	CalculerPariteMixte();
                });

                $('#tab2').on('change','#Prdt,#TypeOp,#PariteStock,#RcltStock', function(){
                	ComboRedevancesMixte($('#Prdt').val(), $('#TypeOp').val(), $('#RcltStock').val(), $('#PariteStock').val(), '#Redev', 0);
                }); 
                ...
                $('#wFrm').on('click','#btn',function(){
					...
					if($('#CampCV').val() == '0') {
                        Msg = "Vous devez choisir la CV";
                        Err +=1;
					}
					if($('#RcltStock').val() == '0') {
                        Msg = "Vous devez choisir le Stock";
                        Err +=1;
					}
					if($('#PariteStock').val() == '0') {
                        Msg = "Vous devez choisir la parite du Stock";
                        Err +=1;
					}
					if($('#PariteCV').val() == '0') {
                        Msg = "Vous devez choisir la parité de la CV";
                        Err +=1;
					}
					if($('#TypeOp').val() == '0') {
                    ...
                    if(Err != 0) {
                        ...
                        swal({
                                title: 'Saisie differentiel mixte',
                                ...
                                function(){
                                    _differentiel_mixte_maj('#FrmDiff');
                            });
                        }
                        return false;
                });

                function CalculerPariteMixte(){

                	var PariteCv = $('#PariteCV').val(), PariteStock = $('#PariteStock').val(), Parite = 0;
                	var CampCv = $('#CampCV').val(), RecolteStock = $('#RcltStock').val();
                	
                	CampCv = CampCv*1 + 1;
                	var DifferenceCampRecolte = (parseInt(CampCv) - parseInt(RecolteStock));

                	// - ##### - Campagne CV = Recolte Stock
                	if(DifferenceCampRecolte == 0){
                		if(PariteCv == PariteStock) Parite = PariteCv;
                		else if(PariteCv > PariteStock) Parite = 31;
                		else Parite = 32;
                	}
					
					// - ##### - Campagne CV < Recolte Stock
                	if(DifferenceCampRecolte < 0){
                		if((PariteCv > PariteStock) || (PariteCv == PariteStock && PariteCv == 1)) Parite = 33;
                		else if((PariteCv < PariteStock) || (PariteCv == PariteStock && PariteCv == 2)) Parite = 34;
                	}

					// - ##### - Campagne CV > Recolte Stock
					if(DifferenceCampRecolte > 0 && PariteStock == 2){
                		Parite = 3;
                	}

                	$('#PariteMixte').val(Parite); 
                }
